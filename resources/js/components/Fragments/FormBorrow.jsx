...
const FormBorrow = ({ onClose, book }) => {
    ...
    const handlePinjam = () => {
        ...
        if (book.status.toLowerCase() === "dipinjam") {
            setAlert({
                show: true,
            ...
            setTimeout(
                () => setAlert({ show: false, message: "", action: "" }),
                2000
            );
            return;
        }

        // Gunakan data dari useForm dengan benar
        post("/borrow-book", {
            book_id: data.book_id, // Ambil dari state useForm
